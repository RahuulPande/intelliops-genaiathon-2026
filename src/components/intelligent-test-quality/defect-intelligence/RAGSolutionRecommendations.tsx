...
export default function RAGSolutionRecommendations() {
  ...
  return (
    <div className="p-6 space-y-6">
            ...
            return (
              <div key={step.step} className="flex items-center flex-1">
                ...
                >
                  ...
                  <div className={`text-xs font-medium mt-2 ${isActive ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-600'}`}>
                    {step.label}
                  </div>
                  <div className={`text-xs ${isActive ? 'text-gray-500 dark:text-gray-400' : 'text-gray-300 dark:text-gray-600'}`}>
                    {step.description}
                  </div>
                </motion.div>
      ...
      {/* Active Defect (Query) */}
      <div className="bg-white dark:bg-[#1A1A1A] rounded-xl border-2 border-blue-200 dark:border-blue-900 p-6">
        <div className="flex items-center space-x-2 mb-4">
          <div className="bg-blue-100 p-1.5 rounded-lg">
            <AlertTriangle className="w-5 h-5 text-blue-600" />
          </div>
          <h3 className="font-semibold text-gray-900 dark:text-white">Active Defect — Input Query</h3>
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <div className="flex items-center space-x-2 mb-1">
              ...
              <span className="text-xs text-gray-500 dark:text-gray-400">{activeDefect.module}</span>
              <span className="text-xs text-gray-400 dark:text-gray-600">•</span>
              <span className="text-xs text-gray-500 dark:text-gray-400">{activeDefect.created}</span>
            </div>
            <h4 className="text-lg font-semibold text-gray-900 dark:text-white mb-2">{activeDefect.title}</h4>
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">{activeDefect.description}</p>
      ...
      <div className="bg-white dark:bg-[#1A1A1A] rounded-xl border border-gray-200 dark:border-gray-800 p-6">
        <div className="flex items-center space-x-2 mb-4">
          ...
          <h3 className="font-semibold text-gray-900 dark:text-white">Retrieved Similar Defects</h3>
        ...
        <div className="space-y-3">
          {similarDefects.map((defect, index) => (
            <motion.div
              ...
              className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 hover:border-indigo-300 dark:hover:border-indigo-700 hover:shadow-sm dark:hover:shadow-none transition-all"
            >
              <div className="flex items-start justify-between mb-2">
                <div className="flex-1">
                  <div className="flex items-center space-x-2 mb-1">
                    <span className="text-xs font-mono text-indigo-600 dark:text-indigo-400">{defect.id}</span>
                    <span className="text-xs text-gray-400 dark:text-gray-600">{defect.module}</span>
                    <span className="text-xs text-gray-400 dark:text-gray-600">•</span>
                    <span className="text-xs text-gray-400 dark:text-gray-600">Resolved: {defect.resolvedDate}</span>
                    <span className="text-xs text-gray-400 dark:text-gray-600">•</span>
                    <span className="text-xs text-gray-400 dark:text-gray-600">MTTR: {defect.mttr}</span>
                  </div>
                  <h4 className="text-sm font-medium text-gray-900 dark:text-white">{defect.title}</h4>
                </div>
      ...
      {/* AI-Generated Recommendation */}
      <div className="bg-white dark:bg-[#1A1A1A] rounded-xl border-2 border-green-200 dark:border-green-900 p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-2">
            <div className="bg-green-100 p-1.5 rounded-lg">
              <Lightbulb className="w-5 h-5 text-green-600" />
            </div>
            <h3 className="font-semibold text-gray-900 dark:text-white">AI-Generated Resolution Recommendation</h3>
          </div>
          <div className="flex items-center space-x-2">
        ...
        {/* Root Cause Analysis */}
        <div className="bg-red-50 dark:bg-red-900/20 rounded-lg p-4 mb-4 border border-red-200 dark:border-red-900">
          <h4 className="font-semibold text-red-800 dark:text-red-200 mb-2 flex items-center space-x-2">
            <Target className="w-4 h-4" />
            <span>Root Cause Analysis</span>
          </h4>
          <p className="text-sm text-red-700 dark:text-red-200 font-medium mb-2">{aiRecommendation.rootCauseAnalysis.primaryCause}</p>
          <div className="space-y-1 mb-3">
            {aiRecommendation.rootCauseAnalysis.contributingFactors.map((factor, i) => (
              <div key={i} className="flex items-start space-x-2 text-xs text-red-600 dark:text-red-300">
                <span className="mt-0.5">•</span>
                <span>{factor}</span>
              </div>
            ))}
          </div>
          <div className="bg-white/50 dark:bg-white/10 rounded p-2 text-xs text-red-600 dark:text-red-300 italic">
          ...
          {aiRecommendation.suggestedFixes.map((fix) => (
            ...
            >
              <button
                ...
                className="w-full flex items-center justify-between p-4 hover:bg-gray-50 dark:hover:bg-[#242424] transition-colors"
              >
                <div className="flex items-center space-x-3">
                  ...
                  <div className="text-left">
                    <h5 className="font-medium text-gray-900 dark:text-white">{fix.title}</h5>
                ...
                <ArrowRight className={`w-4 h-4 text-gray-400 dark:text-gray-600 transition-transform ${expandedFix === fix.priority ? 'rotate-90' : ''}`} />
              </button>
              ...
              {expandedFix === fix.priority && (
                <motion.div
                  ...
                  className="px-4 pb-4"
                >
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-3">{fix.description}</p>

                  <div className="bg-gray-900 rounded-lg p-4 mb-3 relative">
                    <button className="absolute top-2 right-2 text-gray-400 dark:text-gray-600 hover:text-white dark:hover:text-gray-200">
                    ...
                    <div className="flex items-center space-x-2">
                      <button
                        ...
                        className={`p-1.5 rounded transition-colors ${
                          feedbackGiven[fix.priority] ? 'bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-300' : 'text-gray-400 dark:text-gray-600 hover:bg-gray-100 dark:hover:bg-[#242424]'
                        }`}
                      >
                        <ThumbsUp className="w-4 h-4" />
                      </button>
                      <button className="p-1.5 rounded text-gray-400 dark:text-gray-600 hover:bg-gray-100 dark:hover:bg-[#242424] transition-colors">
        ...
        <div className="mt-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4 border border-blue-200 dark:border-blue-900">
          ...
          <div className="space-y-1">
            {aiRecommendation.testRecommendations.map((test, i) => (
              <div key={i} className="flex items-center space-x-2 text-sm text-blue-700 dark:text-blue-200">
      ...
      <div className="bg-gray-50 dark:bg-[#141414] rounded-lg p-4 border border-gray-200 dark:border-gray-800">
        <div className="flex items-center space-x-2 mb-3">
          <BookOpen className="w-4 h-4 text-gray-600 dark:text-gray-400" />
          <span className="text-sm font-medium text-gray-700 dark:text-gray-300">RAG Knowledge Sources Used</span>
        </div>
        <div className="flex flex-wrap gap-2">
          {['JIRA Defect History (3,247 tickets)', 'Resolution Wiki (680 articles)', 'Git Commit Messages (45K)', 'Stack Traces DB (8,200)', 'Post-Mortem Reports (156)'].map(source => (
            <span key={source} className="text-xs bg-white dark:bg-[#242424] border border-gray-200 dark:border-gray-700 rounded-full px-3 py-1 text-gray-600 dark:text-gray-400">
              {source}
            </span>
          ))}
        </div>
